...
import { MainImageCard } from '../ReusableComponents/MainImageCard';
...
/* import RenderCarousel from '../ReusableComponents/RenderCarousel'; */
...
import { AWS_URL, MEETING_PROPERTY_LIST , PROPERTY_LIST, ADD_TO_WISHLIST, third_party_text} from '../../config/RestAPI';
...
export default class MeetingSpaceDetails extends Component {
    ...
    componentDidMount = async () => {
        ...
        this._getPropertyInfo(this.props.route.params.params);
    ...
    _getPropertyInfo = async (params) => {
        ...
        getMeetingSpaceDetails(params).then((result) => {
            ...
            if (result.status) {
                ...
                let carousel_images = temp_array.property_images.length > 0 ? CommonHelpers.processRawImages(temp_array.property_images) : [];

                let carousel_images_base = []



                temp_array.property_images.length != 0 ?

                    temp_array.property_images.forEach(item => {
                ...
                temp_array.property_amenities.map((amenity, index) => {
                ...
                this.setState({
                    ...
                    image_path: temp_array.property_images.length != 0 ? AWS_URL + temp_array.property_images[0].image_path : null,

                    carousel_images: this.props.route.params.resource_images == null ?carousel_images : [...this.props.route.params.resource_images, ...carousel_images],//temp_array.images.length != 0? temp_array.images: [],

                    carousel_images_base: carousel_images_base,

                    resource_plan: temp_array.resource_plan,

                    amenity: temp_array.property_amenities,
                ...
                result.dataArray.map((item, index) => {
                   ...
                   /*  if (item.id == this.state.API_DATA[0].id) {
                    ...
                    const mapItem = {
                        ...
                        imageURL: item.images == undefined || ( item.images == null && item.images.length == 0 ) ?require('./../../Assets/images/BRHlogoorange.png') : {uri:CommonHelpers.processRawImages(item.images)[0].url},
    ...
    bookNow = (item) => {
        ...
        //console.log('booknow',item)

        if(item.access_period_unit_id == 3 && !this.state.isLogin){

            Session.setMeetingSpacesData(JSON.stringify({

                item: item,

                API_DATA:this.state.API_DATA.length != 0 ?this.state.API_DATA[0] : {},

                property_name: this.state.property_name,

                property_timings: this.state.API_DATA.length != 0 ? this.state.API_DATA[0].property_timings: [],

                addons: this.state.addons.length == 0? []: this.state.addons,
            ...
            this.props.navigation.push('bookingmeetingspaces', {
        ...
        return (
            ...
            <TouchableOpacity onPress={() => {
                ...
                <MeetingCard
                    ...
                    source={item.images != null && item.images.length != 0 ? { uri: AWS_URL + item.images[0].image_path } : require('../../Assets/images/BRHlogoorange.png')}
                        ...
                        <ScrollView contentContainerStyle={{ flexGrow: 1, }}>
...

                            {
                                ...
                                >
                                    ...
                                    <View height={ this.state.long_desc.replace(/\s+/g, ' ').split(' ').length / 6 * 12 + 10}>
                                        ...
                                        <Text style={[
                                           ...
                                           ellipsizeMode="tail"
                                            numberOfLines={Math.ceil(this.state.long_desc.replace(/\s+/g, ' ').split(" ").length / 6)}

                                        >
                                            {this.state.long_desc.replace(/\s+/g, ' ')}
                                        </Text>
